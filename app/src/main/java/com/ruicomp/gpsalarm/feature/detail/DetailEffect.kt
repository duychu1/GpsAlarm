...
sealed class DetailEffect : Reducer.ViewEffect {
    data class NavigateToMaps(val id: Int?, val lat: Double?, val lng: Double?, val radius: Int, val addressLine: String?) : DetailEffect()
    data class NavigateToScreen(val screen: String) : DetailEffect()
    data class ShowToasts(val msg: String): DetailEffect()
}